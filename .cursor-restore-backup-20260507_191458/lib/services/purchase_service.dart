...
/// Підписка / entitlements (один PRO-рівень у IAP).
...
class PurchaseService {
  ...
  AppTier get tier => isPremium ? AppTier.pro : AppTier.free;

  String get proMonthlyId => _premiumProductId;

  void _syncTierNotifier() {
    tierNotifier.value = isPremium ? AppTier.pro : AppTier.free;
  }

  /// Покупка відповідного рівня (зараз лише [AppTier.pro] через IAP).
  Future<bool> buyTier(AppTier target) async {
    if (target == AppTier.free) return true;
    return buyPremium();
  }
  ...
  Future<void> _handlePurchase(PurchaseDetails purchaseDetails) async {
    if (purchaseDetails.status == PurchaseStatus.pending) {
      ...
      if (purchaseDetails.status == PurchaseStatus.restored) {
        ...
        debugPrint(
          '✅ Purchase restored from store: ${purchaseDetails.productID}',
        );
  ...
  Future<bool> _verifyPurchaseOnServer(PurchaseDetails purchaseDetails) async {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (data['pending'] == true) {
            debugPrint('⏳ Purchase is still pending in store verification');
          }
          return false;
        }
        ...
        if (response.statusCode == 503) {
          debugPrint(
            '⚠️ Purchase verification temporarily unavailable; trusting store purchase',
          );
          return true;
        }
    ...
    // The user already completed the native store flow. Do not strand a paid
    // purchase because our verifier is temporarily unreachable.
    debugPrint(
      '⚠️ Server verification unreachable after $maxRetries attempts — '
      'trusting store purchase for ${purchaseDetails.productID}',
    );
    return true;
  }

  Future<bool> _ensureStoreAvailable() async {
    try {
      final available = await _inAppPurchase.isAvailable();
      if (!available) {
        onPurchaseError?.call('Магазин покупок тимчасово недоступний');
      }
      return available;
    } catch (e) {
      debugPrint('Store availability check failed: $e');
      onPurchaseError?.call('Не вдалося відкрити магазин покупок');
      return false;
    }
  }
  ...
  Future<bool> buyPremium() async {
    ...
    if (!await _ensureStoreAvailable()) {
      return false;
    }
    if (products.isEmpty) {
