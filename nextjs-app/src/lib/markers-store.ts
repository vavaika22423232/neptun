...
import { isTrackAdminDestroyed, loadSettings } from './admin/data';
...
export async function upsertByTrackId(
  trackId: string,
  marker: Record<string, unknown>,
): Promise<{ mode: 'created' | 'updated' | 'suppressed'; id: string; total: number }> {
  return withWriteLock(async () => {
    const s = getState();
    if (isTrackAdminDestroyed(trackId)) {
      return { mode: 'suppressed', id: trackId, total: s.messages.length };
    }
...
function pruneMessages(messages: Record<string, unknown>[]): Record<string, unknown>[] {
  ...
  let result = messages.filter((m) => {
    ...
    const ts = coerceTimestampString(m.ts || m.timestamp || m.date);
    if (ts) {
  ...
  if (result.length > MAX_MESSAGES) {
    ...
    result.sort((a, b) => {
      ...
      const aMs = aEpoch && aEpoch > 1000000000
        ...
        : new Date(coerceTimestampString(a.ts || a.timestamp || a.date)).getTime() || 0;
      const bMs = bEpoch && bEpoch > 1000000000
        ? (bEpoch > 10000000000 ? bEpoch : bEpoch * 1000)
        : new Date(coerceTimestampString(b.ts || b.timestamp || b.date)).getTime() || 0;
...
function coerceTimestampString(value: unknown): string {
