  ...
  return withWriteLock(async () => {
    ...
    if (matchIdx >= 0) {
      ...
      if (doBroadcast) {
        ...
        if (ingestShouldBroadcastMarker(existing as Record<string, unknown>)) {
...
function legacyTimestampMs(value: unknown): number {
...
function pruneMessages(messages: Record<string, unknown>[]): Record<string, unknown>[] {
  ...
  let result = messages.filter((m) => {
    ...
    const parsed = legacyTimestampMs(m.ts || m.timestamp || m.date);
    if (parsed > 0) {
  ...
  if (result.length > MAX_MESSAGES) {
    ...
    result.sort((a, b) => {
      ...
      const aMs = aEpoch && aEpoch > 1000000000
        ...
        : legacyTimestampMs(a.ts || a.timestamp || a.date);
      const bMs = bEpoch && bEpoch > 1000000000
        ? (bEpoch > 10000000000 ? bEpoch : bEpoch * 1000)
        : legacyTimestampMs(b.ts || b.timestamp || b.date);
