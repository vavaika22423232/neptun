...
export async function POST(request: Request) {
  try {
    ...
    if (!allowed) {
      logSecurityEvent('rate_limit_hit', { route: 'purchase_apple_verify' });
    ...
    const result = await verifyAndBindPurchase({
      ...
      purchaseToken: body?.purchaseToken ?? body?.purchase_token ?? body?.receipt,
      source: 'app_store',
      platform: body?.platform ?? 'ios',
      appVersion: body?.appVersion,
      expiresAt: body?.expiresAt ?? null,
      originalTransactionId: body?.originalTransactionId ?? body?.transactionId ?? null,
    });

    if (!result.ok) {
      return NextResponse.json(
        { valid: false, pending: result.reason === 'pending', transient: result.reason === 'transient', error: result.reason },
    ...
    console.error('[v1/purchases/apple/verify]', e);
