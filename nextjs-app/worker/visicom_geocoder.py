...
OBLAST_KEYS = {
    'київ': ['київ', 'київська', 'київщина', 'м. київ'],
    'харків': ['харків', 'харківська', 'харківщина'],
    'одес': ['одеса', 'одеська', 'одещина'],
    'дніпр': ['дніпро', 'дніпропетровська', 'дніпропетровщина', 'дніпропетров'],
    'запоріж': ['запоріжжя', 'запорізька', 'запоріж', 'запорожжя'],
    'львів': ['львів', 'львівська', 'львівщина'],
    'миколаїв': ['миколаїв', 'миколаївська', 'миколаївщина'],
    'херсон': ['херсон', 'херсонська', 'херсонщина'],
    'полтав': ['полтава', 'полтавська', 'полтавщина'],
    'сум': ['суми', 'сумська', 'сумщина'],
    'чернігів': ['чернігів', 'чернігівська', 'чернігівщина'],
    'вінниц': ['вінниця', 'вінницька', 'вінниччина'],
    'житомир': ['житомир', 'житомирська', 'житомирщина'],
    'черкас': ['черкаси', 'черкаська', 'черкащина'],
    'кропивниц': ['кропивницький', 'кіровоградська', 'кіровоградщина', 'кіровоград'],
    'донец': ['донецьк', 'донецька', 'донеччина'],
    'луганськ': ['луганськ', 'луганська', 'луганщина'],
    'хмельниц': ['хмельницький', 'хмельницька', 'хмельниччина'],
    'рівн': ['рівне', 'рівненська', 'рівненщина'],
    'волин': ['волинь', 'волинська', 'волинщина', 'луцьк'],
    'тернопіл': ['тернопіль', 'тернопільська', 'тернопільщина'],
    'івано-франків': ['івано-франківськ', 'івано-франківська', 'івано-франківщина'],
...
OBLAST_BBOX = {
    'київ': (48.5, 52.5, 28.5, 33.0),
    'харків': (48.0, 51.0, 34.5, 39.0),
    'одес': (44.5, 49.0, 28.0, 31.5),  # max_lng: exclude Kryvyi Rih (33.39°)
    'дніпр': (47.0, 50.0, 32.5, 37.0),  # min_lng: include Kryvyi Rih (33.39°)
    'запоріж': (46.0, 49.0, 33.5, 37.5),
    'львів': (48.0, 51.0, 22.0, 26.0),
    'миколаїв': (45.5, 49.0, 30.0, 34.0),
    'херсон': (45.0, 48.0, 31.5, 36.0),
    'полтав': (48.0, 51.0, 32.0, 36.0),
    'сум': (49.5, 53.0, 31.5, 36.5),
    'чернігів': (50.0, 53.0, 29.5, 34.0),
    'вінниц': (47.5, 50.5, 26.5, 30.5),
    'житомир': (49.0, 52.0, 26.5, 31.0),
    'черкас': (48.0, 50.5, 30.0, 33.5),
    'кропивниц': (47.0, 50.0, 30.5, 34.5),
    'донец': (46.5, 50.0, 36.0, 39.5),
    'луганськ': (47.0, 50.5, 37.5, 41.0),
    'хмельниц': (48.0, 51.0, 25.0, 29.0),
    'рівн': (49.5, 52.5, 24.5, 28.0),
    'волин': (50.0, 53.0, 23.0, 26.5),
    'тернопіл': (48.0, 51.0, 24.0, 27.0),
    'івано-франків': (47.0, 50.0, 23.0, 26.0),
    'закарпат': (47.0, 50.0, 21.5, 25.0),
    'чернівц': (47.0, 49.5, 24.0, 27.5),
    'крим': (43.5, 47.0, 32.0, 37.0),
}
...
SPECIAL_NAME_MAPPINGS = {
    ...
    # БПЛА Білика/Ріпка/Межів — case forms so results stay in correct oblast
    "білика": "Білики",   # Білики (Полтавська обл.)
    "ріпка": "Ріпки",     # Ріпки (Чернігівська обл.)
    "межів": "Межова",    # Межова (Дніпропетровська обл.)
}
...
# Use /data for persistent storage on VPS
...
# Normalize -щина/-ччина to -ська for consistent cache keys (parser may send "сумщина", we want "сумська")
_REGION_ЩИНА_TO_СЬКА = {
    'сумщина': 'сумська', 'чернігівщина': 'чернігівська', 'київщина': 'київська', 'полтавщина': 'полтавська',
    'дніпропетровщина': 'дніпропетровська', 'харківщина': 'харківська', 'миколаївщина': 'миколаївська',
    'одещина': 'одеська', 'херсонщина': 'херсонська', 'черкащина': 'черкаська', 'вінниччина': 'вінницька',
    'житомирщина': 'житомирська', 'рівненщина': 'рівненська', 'волинщина': 'волинська', 'львівщина': 'львівська',
    'донеччина': 'донецька', 'луганщина': 'луганська', 'тернопільщина': 'тернопільська',
    'хмельниччина': 'хмельницька', 'івано-франківщина': 'івано-франківська', 'кіровоградщина': 'кіровоградська',
}

def _normalize_key(city: str, region: str = None) -> str:
    """Create cache key from city and region. Normalize region to one form (e.g. сумська) for key consistency."""
    ...
    region_clean = region_clean.replace(' область', '').replace(' обл.', '').replace(' обл', '')
    region_clean = region_clean.strip()
    # Unify -щина/-ччина to -ська so "Сумська область" and "сумщина" yield same key
    region_clean = _REGION_ЩИНА_TO_СЬКА.get(region_clean, region_clean)
...
def visicom_geocode(city: str, region: str = None) -> tuple:
    ...
    _raw = city.strip()
    _stripped = strip_settlement_prefix(_raw)
    city = _stripped if _stripped else _raw
    ...
    SKIP_WORDS = {
        ...
        'швидкість', 'висота',
        # Non-place words (alert template verbs/adverbs)
        'негайно', 'перейдіть', 'залишайтесь', 'залишайтеся',
        'прямуйте', 'зверніть', 'будьте', 'слідкуйте',
        'сховайтесь', 'сховайтеся', 'покиньте', 'виходьте',
        'терміново', 'обережно', 'небезпечно', 'небезпека',
        'загроза', 'загрози', 'загрозу', 'тривога',
        'попередження', 'повідомлення', 'інформація',
        'повітряна', 'очікується', 'підтверджено', 'зафіксовано',
        'оголошено', 'скасовано', 'відмінено', 'відбій',
        'укриття', 'евакуюйтесь', 'евакуюйтеся',
        'авіація', 'авіації', 'тактичної', 'тактична',
        'обстріл', 'обстріли', 'територіальна', 'громада',
        'місто', 'міста', 'містом', 'містах', 'містечко',
        'район', 'району', 'районі', 'село', 'села', 'селище',
        'область', 'області', 'центр', 'центру', 'околиці',
    }
