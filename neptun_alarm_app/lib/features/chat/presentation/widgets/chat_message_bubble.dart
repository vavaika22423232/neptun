...
class _ChatMessageBubbleState extends State<ChatMessageBubble> {
                  ...
                  Padding(
                    ...
                    child: Text(
                      ...
                      style: GoogleFonts.inter(
                        ...
                        color: cs.onSurface.withValues(alpha: isDark ? 0.3 : 0.45),
                      ),
                    ),
                  ),
                if (widget.isMine) ...[
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: Icon(
              widget.isPlayingVoice ? Icons.pause_rounded : Icons.play_arrow_rounded,
              color: cs.primary,
              size: 22,
            ),
          ),
  ...
  Widget _buildMessageText(String text) {
    ...
    if (hasHighlight) {
      try {
        highlightMatches = RegExp(RegExp.escape(highlight), caseSensitive: false)
            .allMatches(text)
            .toList();
      } catch (_) {
        highlightMatches = [];
    ...
    if (urlMatches.isEmpty && (highlightMatches == null || highlightMatches.isEmpty)) {
      ...
      if (r.isUrl) {
        spans.add(
          TextSpan(
            ...
            recognizer: TapGestureRecognizer()..onTap = () => _launchUrl(segment),
          ),
        );
      } else {
        spans.add(
    ...
    return Padding(
      ...
      child: Wrap(
        ...
        children: widget.message.reactions.entries.map((e) {
          final hasReacted =
              widget.message.hasReacted(e.key, widget.myDeviceId);
