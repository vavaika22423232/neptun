...
class PremiumNotifier extends Notifier<PremiumState> {
  ...
  String? _priceFromProducts(PurchaseService svc) {
    ...
    final matched = svc.products
        .where((p) => p.id == PurchaseService.proForeverId)
        .toList();
  ...
  Future<void> buyTier(String productId) async {
    ...
    svc.onPurchaseSuccess = () {
      finish();
      if (ref.mounted) state = state.copyWith(showThankYou: true);
    };
    ...
    try {
      final started = await svc.buyTier(productId);
      if (!started) {
        finish();
        if (ref.mounted) {
          state = state.copyWith(
  ...
  /// Alias — відкриває одноразову покупку PRO.
  Future<void> buyPremium() => buyTier(PurchaseService.proForeverId);
