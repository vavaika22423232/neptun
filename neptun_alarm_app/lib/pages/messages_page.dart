  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) {
      return SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const NeptunShimmer(width: double.infinity, height: 44, borderRadius: 12),
            const SizedBox(height: 24),
            Row(
              children: [
                Expanded(child: NeptunShimmer(width: double.infinity, height: 56, borderRadius: 12)),
            ...
            ...List.generate(5, (_) => const Padding(
              ...
              child: NeptunShimmer(width: double.infinity, height: 72, borderRadius: 14),
            )),
          ],
        ),
      );
    ...
    return Column(
      children: [
        // Hero status bar
        Padding(
          padding: EdgeInsets.fromLTRB(NeptunSpacing.lg, topInset, NeptunSpacing.lg, 0),
          child: TacticalSurface(
            style: TacticalSurfaceStyle.flat,
            padding: const EdgeInsets.all(NeptunSpacing.lg),
            child: Row(
              children: [
                Icon(Icons.notifications_active_rounded, color: cs.primary, size: 22),
                const SizedBox(width: NeptunSpacing.md),
                Expanded(
                  child: Text(
                    _selectedOblastCount > 0
                        ? 'Обрано $_selectedOblastCount обл.'
                        : 'Оберіть регіони для сповіщень',
                ...
                StatusPill(
                  ...
                  variant: _selectedRegions.isNotEmpty ? StatusPillVariant.safe : StatusPillVariant.neutral,
        ...
        Padding(
          padding: const EdgeInsets.fromLTRB(NeptunSpacing.lg, NeptunSpacing.lg, NeptunSpacing.lg, 0),
          child: _buildSearchBar(cs),
        ),
        Padding(
          padding: const EdgeInsets.fromLTRB(NeptunSpacing.lg, NeptunSpacing.sm, NeptunSpacing.lg, NeptunSpacing.md),
          child: Row(
            children: [
              Expanded(
                child: _quickButton(label: 'Обрати всі', icon: Icons.select_all_rounded, cs: cs, onTap: _selectAll),
              ),
              const SizedBox(width: NeptunSpacing.sm),
              Expanded(
                child: _quickButton(label: 'Скинути', icon: Icons.deselect_rounded, cs: cs, onTap: _clearAll),
              ),
            ],
          ),
        ),
        SectionHeader(title: 'Регіони', icon: Icons.map_rounded),
        Expanded(
          child: RefreshIndicator(
            onRefresh: _loadSettings,
            child: ListView.builder(
              padding: const EdgeInsets.fromLTRB(NeptunSpacing.lg, 0, NeptunSpacing.lg, NeptunSpacing.xxxl),
              itemCount: _filteredOblasts.length,
              itemBuilder: (ctx, i) => _buildOblastCard(_filteredOblasts[i], cs),
            ),
          ),
        ),
      ],
    ...
    return GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(NeptunRadius.md),
            border: Border.all(
              color: isDark ? NeptunSurfaces.border : cs.outline.withValues(alpha: 0.3),
              width: 0.5,
            ),
          ),
          child: Row(
