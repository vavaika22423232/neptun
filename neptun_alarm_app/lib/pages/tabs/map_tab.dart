...
class _MapTabState extends State<MapTab>
  ...
  Future<void> _loadInitialRequest(WebViewController ctrl) async {
    final mapUrl = _buildMapUrl();
    await ModeratorService.instance.init();
    if (!ModeratorService.instance.isModerator) {
      await ctrl.loadRequest(Uri.parse(mapUrl));
      return;
    }
    ...
    final bootstrapUrl = Uri.parse(
      'https://neptun.in.ua/api/admin/auth/embed?next=${Uri.encodeComponent(mapUrl.replaceFirst('https://neptun.in.ua', ''))}',
    );
  ...
  void _checkThemeAndReload() {
    ...
    if (_lastTheme != null && _lastTheme != isDark && _controller != null) {
      ...
      _controller!.runJavaScript(
        'if(window.setTheme) window.setTheme("$themeStr");',
      );
    }
  }
