...
class _ChatTabState extends State<ChatTab> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> _send() async {
    ...
    final optimistic = ChatMessage(
      ...
      replyTo: replyTo != null
          ? ReplyInfo(id: replyTo.id, nickname: replyTo.userId, text: replyTo.message)
          : null,
      isPro: PurchaseService().isPremium,
    );
    if (mounted) {
  ...
  Future<void> _onAttach() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Помилка відправки фото'), behavior: SnackBarBehavior.floating),
        );
      }
    }
    if (mounted) setState(() => _isSending = false);
  ...
  Widget _buildAgeGate() {
    ...
    final maxBirthYear = currentYear - 5; // включно молодші, щоб було що відсікати
            ...
            child: Column(
              ...
              children: [
                Icon(Icons.cake_outlined,
                    size: 56, color: cs.primary.withValues(alpha: 0.8)),
                const SizedBox(height: 24),
                Text(
...
''';
    ...
    return Container(
      ...
      child: _searchMode
          ...
          : StreamBuilder<bool>(
              ...
              builder: (_, snap) => _buildNormalHeader(cs),
            ),
    );
  }

  Widget _buildNormalHeader(ColorScheme cs) {
    final isMod = ModeratorService.instance.isModerator;
    return Row(
      children: [
        GestureDetector(
          onTap: () {
        ...
        GestureDetector(
          onTap: () {
            if (isMod) {
              ...
              _showModeratorRequiredDialog();
            }
          },
          child: Icon(
            Icons.admin_panel_settings_rounded,
            color: isMod ? cs.tertiary : cs.onSurfaceVariant,
            size: 22,
          ),
        ),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isFirstInGroup =
            nextMsg == null || nextMsg.userId != msg.userId;
        final isLastInGroup =
        ...
        final bubble = ChatMessageBubble(
          ...
          searchHighlight: _searchQuery.isNotEmpty &&
  ...
  void _showModeratorRequiredDialog() {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Адмін панель чату'),
        content: const Text(
          'Доступ лише для модераторів.\n\n'
          'Натисніть ⚙ → розгорніть «Модерація» → введіть пароль.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
